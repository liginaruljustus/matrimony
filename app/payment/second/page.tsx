...
import {
  ...
  ArrowLeft, Smartphone, Building2, QrCode, Phone,
} from "lucide-react";
...
function SecondPaymentContent() {
  ...
  const brideName   = params.get("name") ?? "this bride";
  ...
  const handleSubmit = async () => {
    if (!favoriteId) { setError("Invalid link — missing profile ID"); return; }
  ...
  if (!favoriteId) {
    return (
      <div className="mx-auto max-w-md px-4 py-16 text-center">
        ...
        <h1 className="text-xl font-bold text-neutral-800">Invalid Link</h1>
        <p className="mt-2 text-sm text-neutral-500">No profile was specified. Go back to your Inbox.</p>
        <Link href="/inbox" className="mt-6 inline-block rounded-lg bg-[#7a1f2b] px-6 py-3 text-sm font-bold text-white hover:bg-[#6b1823] transition-colors">
          Go to Inbox
        </Link>
      </div>
    );
    ...
    return (
      <div className="mx-auto max-w-md px-4 py-16 text-center">
        ...
        <p className="mt-3 text-sm text-neutral-500">
          Your 2nd payment for <strong>{brideName}</strong> is under admin review.
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-8">
      ...
      <p className="mt-1 text-sm text-neutral-500">
        Unlock contact details for <strong>{brideName}</strong>
      </p>
      ...
      <div className="mt-6 rounded-xl border-2 border-[#7a1f2b]/20 bg-[#faf7f2] p-5">
        ...
        <p className="mt-0.5 text-xs text-neutral-400">
          Based on family class tier of {brideName}
        </p>
      </div>
        ...
        {(method === "gpay" || method === "upi") && (
          <div className="space-y-3">
            ...
            <p className="text-xs text-neutral-400">
              Open Google Pay / PhonePe / any UPI app, send ₹{rawAmount.toLocaleString("en-IN")} to the UPI ID above,
